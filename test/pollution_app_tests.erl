...
getOneValue() ->
  ...
  [?_assert(Value =:= {ok, 45}),
    ?_assert(ErrorNoStation =:= {error, no_station}),
    ?_assert(ErrorNoValue =:= {error, no_value})].
...
getStationWithTheMostMeasurements() ->
  ...
  [?_assert(ErrorEmptyMonitor =:= {error, empty_monitor}),
    ?_assert(Station == {ok, {2, 2}})].
...
getNearestStation() ->
  ...
  [?_assert(ErrorNoStation =:= {error, no_station}),
    ?_assert(ErrorJustOneStation =:= {error, just_one_station}),
    ?_assert(Station1 == {ok, {1, 3}}),
    ?_assert(Station2 == {ok, {3, 3}}),
    ?_assert(Station3 == {ok, {3, 2}})].

getNearestStation_test_() ->
  test(fun() -> getNearestStation() end).


%% checking if program is saving state in case of generic server crash
checkCrash() ->
  pollution_gen_server:addStation("Krakow", {1, 1}),
  pollution_gen_server:addValue({1, 1}, {{2020, 04, 02}, {12, 00, 00}}, "PM2.5", 15),
  ?_assertException(error, badarith, pollution_gen_server:crash()).

checkStateAfterCrash() ->
  Value = pollution_gen_server:getOneValue({1, 1}, {{2020, 04, 02}, {12, 00, 00}}, "PM2.5"),
  ?_assert(Value =:= {ok, 15}).
